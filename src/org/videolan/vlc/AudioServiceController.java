...
public class AudioServiceController implements IAudioPlayerControl {
    ...
    private IAudioService mAudioServiceBinder;   //远程调用的接口
     ...
     * 哪里调用了呢？ MainActivity中的 onResume和onPause中 分别绑定和解绑定
     */
    public void bindAudioService(Context context) {
        ...
        if (!mIsBound) {
            ...
            mAudioServiceConnection = new ServiceConnection() {
                ...
                public void onServiceConnected(ComponentName name, IBinder service) {
                    if (!mIsBound) // Can happen if unbind is called quickly before this callback
                        return;		//保证下面bindService的绑定成功后才调用以下内容
                    Log.d(TAG, "Service Connected");
                    //获得远程调用的接口实例！----AudioService类中的mInterface对象！
            ...
            //将service绑定到context，随context的生命周期存亡，ServiceConnection对象得到service开始和结束的反馈，进行相应处理。
     ...
     * 这个方法很诡异。。。通过IAudioService（49行 mAudioServiceBinder实例）实现远程调用，貌似0_o...不过是怎么实现的呢？似乎是RTSP网络数据交互的突破口
     * 
    ...
    public void append(List<String> mediaPathList) {
    	Log.d(TAG, "append "+mediaPathList.get(0));
