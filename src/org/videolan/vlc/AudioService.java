...
public class AudioService extends Service {
    ...
    private Media mCurrentMedia;	//存储当前播放视频，关键！
    ...
    public void onCreate() {
        ...
        //注册广播监听程序serviceReceiver到main线程中，只要有广播的intent符合filter就调用serviceReceiver相关操作
    ...
    //用于返回实现了接口的对象实例，没有显示调用，应该是service内部的机制吧。必须实现！
    @Override
...
/*
    ...
    private void showNotification() {
        try {
            ...
            //每一次调用这个方法都将执行AudioService中的onStartCommand方法！！！这是更新视频的关键，也就是视频播放的原理？！？！好像是更新通知栏。。。？
            startService(new Intent(this, AudioService.class));
            //让服务在前台运行，这样服务就不会被系统停止，当活动停止时，服务才停。
...
//        showNotification();
    ...
    /**
     * 实现了aidl中定义的所有接口。
  	 * IAudioService.aidl先自动编译生成IAudioService.java
   	 * 通过实现IAudioService.java中的Stub抽象类来实现所有接口
     */
...
//            showNotification();
        ...
        public void append(List<String> mediaLocationList) throws RemoteException {
        	Log.d(TAG, "append2 "+mediaLocationList.get(0));
